...
# âââ BANLIST âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
# âââ VIOLATIONS ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ

async def save_violation(chat_id, user_id, username, full_name, reason,
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("""
            ...
            VALUES(?,?,?,?,?,?,?,?,'pending')
        """, (chat_id, user_id, username, full_name, reason,
              message_text, voice_transcript, now))
        await db.commit()
...
async def get_pending_violations():
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
            "SELECT * FROM violations WHERE status='pending' ORDER BY created_at DESC"
        ) as cur:
            return await cur.fetchall()


# âââ WARNINGS ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
# âââ MODERATORS ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
async def save_book(title: str, filename: str, file_path: str) -> int:
    now = datetime.now().isoformat()
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
...
async def update_book_status(book_id: int, status: str, page_count: int = 0):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "UPDATE books SET status=?, page_count=? WHERE id=?",
            (status, page_count, book_id)
        )
        await db.commit()


async def save_book_chunk(book_id: int, book_title: str, page_number: int,
                           chunk_text: str, embedding: str = ""):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "INSERT INTO book_chunks(book_id, book_title, page_number, chunk_text, embedding) "
            "VALUES(?,?,?,?,?)",
            (book_id, book_title, page_number, chunk_text, embedding)
        )
        await db.commit()
...
async def search_chunks_by_text(query: str, limit: int = 5):
    """Simple full-text search over book chunks."""
    words = query.lower().split()
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        results = []
        async with db.execute(
            "SELECT * FROM book_chunks ORDER BY id"
        ) as cur:
...
async def get_random_chunk():
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
            "SELECT * FROM book_chunks ORDER BY RANDOM() LIMIT 1"
        ) as cur:
            return await cur.fetchone()


# âââ BLITZ âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
async def get_random_blitz_question():
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
            "SELECT * FROM blitz_questions ORDER BY used_count ASC, RANDOM() LIMIT 1"
        ) as cur:
            return await cur.fetchone()
...
async def start_blitz_session(question_id, question_text, correct_answer) -> int:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute(
            "UPDATE blitz_sessions SET status='expired', closed_at=? WHERE status='active'",
            (now,)
        )
        cur = await db.execute("""
            INSERT INTO blitz_sessions(question_id, question_text, correct_answer,
                started_at, status)
            VALUES(?,?,?,?,'active')
        """, (question_id, question_text, correct_answer, now))
        await db.execute(
            "UPDATE blitz_questions SET used_count=used_count+1 WHERE id=?", (question_id,)
        )
        await db.commit()
        return cur.lastrowid
...
async def close_blitz_session(session_id: int):
    now = datetime.now().isoformat()
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "UPDATE blitz_sessions SET status='closed', closed_at=? WHERE id=?",
            (now, session_id)
        )
        await db.commit()


async def record_blitz_answer(session_id, user_id, username, answer_text,
                               is_correct, position):
    now = datetime.now().isoformat()
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("""
        ...
        await db.execute(
            "UPDATE users SET blitz_participations=blitz_participations+1 WHERE user_id=?",
            (user_id,)
        )
        await db.commit()


async def get_session_correct_count(session_id: int) -> int:
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            "SELECT COUNT(*) FROM blitz_answers WHERE session_id=? AND is_correct=1",
            (session_id,)
        ) as cur:
            row = await cur.fetchone()
            return row[0] if row else 0
...
async def user_already_answered(session_id: int, user_id: int) -> bool:
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            "SELECT 1 FROM blitz_answers WHERE session_id=? AND user_id=?",
            (session_id, user_id)
        ) as cur:
            return bool(await cur.fetchone())


# âââ FLOOD LOG âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ

async def log_message_time(user_id: int):
    now = datetime.now().isoformat()
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "INSERT INTO flood_log(user_id, message_time) VALUES(?,?)", (user_id, now)
        )
        # clean old records (older than 60s)
        await db.execute(
            "DELETE FROM flood_log WHERE message_time < datetime('now', '-60 seconds')"
        )
        await db.commit()
...
# âââ WEAK TOPICS âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
async def record_weak_topic(user_id: int, topic: str):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if row:
            await db.execute(
                "UPDATE weak_topics SET wrong_count=wrong_count+1, last_wrong=? "
                "WHERE user_id=? AND topic=?",
                (now, user_id, topic)
            )
        else:
            await db.execute(
                "INSERT INTO weak_topics(user_id, topic, last_wrong) VALUES(?,?,?)",
                (user_id, topic, now)
            )
        await db.commit()
...
# âââ DAILY ACTIVITY ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ

async def record_daily_activity(user_id: int, xp_earned: int = 0,
                                  messages: int = 0, correct: int = 0):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if row:
            ...
            await db.execute(
                "INSERT INTO daily_activity(user_id, date, messages, correct, xp_earned) "
                "VALUES(?,?,?,?,?)",
